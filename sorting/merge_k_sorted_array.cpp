...
auto MergeKSortedArrays(const std::vector<ArrayType> &k_arrays) {
    ...
    std::priority_queue<Node, std::vector<Node>, decltype(CompareNode)> heap(CompareNode, first_elements);
    ...
    while(not heap.empty()){
        ...
        if (elem.index < elem.array.size()){
            heap.emplace(elem.array, elem.index + 1);
        }
    }
...
SIMPLE_TEST(MergeKSortedArrays, TestSAMPLE, EXPECTED_DIFF_SIZE_ARRAY, SAMPLE_DIFF_SIZE_ARRAYS);
