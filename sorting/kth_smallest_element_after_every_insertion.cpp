...
auto KthSmallestElementAfterEveryInsertion(const ArrayType stream, const ArrayType::size_type k) {
    ...
    std::priority_queue<ArrayType::value_type, std::greater<ArrayType::value_type> > heap(stream.cbegin(), stream.cbegin() + k);
    std::ArrayType kth_smallest_array(k - 1, 0);
...
SIMPLE_BENCHMARK(KthSmallestElementAfterEveryInsertion, SAMPLE_ARRAYS);

SIMPLE_TEST(KthSmallestElementAfterEveryInsertion, TestSAMPLE, EXPECTED_ARRAY, SAMPLE_ARRAYS);
