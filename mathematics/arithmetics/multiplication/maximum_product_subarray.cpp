...
auto MaximumProductSubarray_TwoWays(const ArrayType &array) {
    ...
    const auto forward_product_pair = MaximumProductSubarrayHelper(array.cbegin(),
                                      array.cend());
    const auto backward_product_pair = MaximumProductSubarrayHelper(array.crbegin(),
                                       array.crend());
    const auto max_product = std::max(forward_product_pair.second,
                                      backward_product_pair.second);
