...
void randomInsertIfNot(const bool criteria, const std::string &char_set, std::string &password) {
    if(not criteria) {
        password.insert(Random_Number<std::string::size_type>(0ul, password.size() - 1ul), 1, RandomChar(char_set));
    }
}
auto FixPassword(const PasswordCriteria &criteria, std::string &password) {
    randomInsertIfNot(criteria.lower_case, PasswordCriteria::LOWERS, password);
    randomInsertIfNot(criteria.upper_case, PasswordCriteria::UPPERS, password);
    randomInsertIfNot(criteria.digit, PasswordCriteria::NUMBERS, password);
    randomInsertIfNot(criteria.special, PasswordCriteria::SPECIALS, password);
    ...
    if(password.size() < PasswordCriteria::REQUIRED_PASSWORD_LENGTH) {
        for(auto i = password.size(); i < PasswordCriteria::REQUIRED_PASSWORD_LENGTH; ++i) {
            randomInsertIfNot(false, PasswordCriteria::LOWERS, password);
        }
    }
}
...
#ifdef WANT_TERMINAL_APP && __linux__
