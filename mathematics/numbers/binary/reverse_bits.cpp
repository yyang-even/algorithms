...
unsigned char ReverseBitsInByte_7(const unsigned char byte) {
    return ((byte * 0x0802LU & 0x22110LU) | (byte * 0x8020LU & 0x88440LU)) * 0x10101LU >>
           16;
}
...
auto ReverseBytes_String(unsigned num) {
    ...
    Reverse_TwoPointers(begin, begin + sizeof(num), [](unsigned char &lhs,
    unsigned char &rhs) {
        std::swap(lhs, rhs);
    });
...
auto ReverseBytes_Shift(const unsigned num) {
    return (num << 24) | (((num >> 16) << 24) >> 16) | (((num << 16) >> 24) << 16) |
           (num >> 24);
}
