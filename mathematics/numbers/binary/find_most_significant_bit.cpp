 ...
 * people on even position are selected. This continues until we are left with one person. Find out
 ...
 */
auto FindMSB(const unsigned num) {
    return (SetAllBitsAfterMSB(num) + 1ull) >> 1;
}


unsigned FindMSB_Float(const unsigned num) {
    return pow(2, static_cast<unsigned>(log2(num)));
}
