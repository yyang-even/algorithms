...
inline constexpr auto CountTrailingZeros_ModulusAndLookup(const uint32_t num) {
    constexpr unsigned Mod37BitPosition[] = {// map a bit value mod 37 to its position
...
inline constexpr auto CountTrailingZeros_MultiplyAndLookup(const uint32_t num) {
    ...
    return num ? MultiplyDeBruijnBitPosition[((uint32_t)((num & -num) * 0x077CB531)) >> 27] : 32;
}


/**
 * @reference   Count number of trailing zeros in Binary representation of a number using Bitset
...
/**
 * @reference   Count number of bits changed after adding 1 to given N
...
/**
 * @reference   Construct the Minimum Bitwise Array I
 ...
 * If it is not possible to find such a value for ans[i] that satisfies the condition, then set ans[i] =
 * -1.
 *
 * @tags    #bit-tricks
 */
auto MinBitwiseArray(const ArrayType &nums) {
    std::vector<int> result;
    for (const auto n : nums) {
        if (n & 1) {
            const auto mask = (((n + 1) ^ n) >> 2) + 1;
            result.push_back(mask ^ n);
        } else {
            result.push_back(-1);
        }
    }
...
SIMPLE_TEST(CountTrailingZeros_ModulusAndLookup,
            TestWorstCase,
            NUM_BITS - 1,
            1 << (NUM_BITS - 1));
...
SIMPLE_TEST(CountTrailingZeros_MultiplyAndLookup,
            TestWorstCase,
            NUM_BITS - 1,
            1 << (NUM_BITS - 1));
...
const ArrayType SAMPLE1 = {2, 3, 5, 7};
const ArrayType EXPECTED1 = {-1, 1, 4, 3};

const ArrayType SAMPLE2 = {11, 13, 31};
const ArrayType EXPECTED2 = {9, 12, 15};


THE_BENCHMARK(MinBitwiseArray, SAMPLE1);

SIMPLE_TEST(MinBitwiseArray, TestSAMPLE1, EXPECTED1, SAMPLE1);
SIMPLE_TEST(MinBitwiseArray, TestSAMPLE2, EXPECTED2, SAMPLE2);
