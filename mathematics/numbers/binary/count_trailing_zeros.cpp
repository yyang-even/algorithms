...
auto CountTrailingZeros_MultiplyAndLookup(const uint32_t num) {
    ...
    return num ? MultiplyDeBruijnBitPosition[((uint32_t)((num & -num) * 0x077CB531U)) >>
                                                              27] : 32;
}
...
MUTUAL_RANDOM_TEST(CountTrailingZeros_Linear, CountTrailingZeros_Parallel, LOWER,
                   UPPER);
...
MUTUAL_RANDOM_TEST(CountTrailingZeros_Linear, CountTrailingZeros_BinarySearch, LOWER,
                   UPPER);
