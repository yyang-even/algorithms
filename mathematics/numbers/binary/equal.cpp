 ...
 * 2) String functions
 */
auto Equal_Xor(const unsigned lhs, const unsigned rhs) {
    return not(lhs ^ rhs);
}


auto Equal_Subtract(const unsigned lhs, const unsigned rhs) {
    return not(lhs - rhs);
}


auto Equal_Complement(const unsigned lhs, const unsigned rhs) {
    return not((lhs & ~rhs));
}
