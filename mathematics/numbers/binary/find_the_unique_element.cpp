 ...
 */
auto FindTheUniqueElementHash(const ArrayType &elements, const ArrayType::size_type K){
    ...
    for(const auto iter = counters.cbegin(); iter != counters.cend(); ++iter){
        if(iter->second == 1u) {
            return iter->first;
        }
    }
...
SIMPLE_BENCHMARK(FindTheUniqueElementHash, SAMPLE1);
...
SIMPLE_BENCHMARK(FindTheUniqueElementBits, SAMPLE1);
