...
auto KAlmostPrimeNumbers_LeastPrimeFactor(const unsigned K, unsigned N) {
    const auto smallest_prime_factors = LeastPrimeFactorOfNumbers(
...
auto KAlmostPrimeNumbers_Counter(const unsigned K, unsigned N) {
    const auto prime_factor_counters = SieveOfEratosthenes_Count(
                                           std::numeric_limits<short>::max());
 ...
 *
 * Given an array of numbers A = {a1, a2 ... an} and the value of k, check if each number ai in A can be expressed
 * as the product of exactly k prime numbers. For every element of the array print 'YES' if the condition is
 * satisfied, else print 'NO'.
