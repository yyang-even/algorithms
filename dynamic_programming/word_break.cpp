    ...
    for (std::string::size_type i = 1; i <= a_string.size(); ++i) {
        ...
        if (words.find(substring) != words.cend()) {
            ...
            if (i == a_string.size()) {
                ...
                AllWayToWordBreak(a_string.substr(i, a_string.size() - i), words, new_prefix + " ",
                                  results);
            }
        }
    }
...
SIMPLE_TEST(AllWayToWordBreak, TestSAMPLE1, EXPECTED1, "ilikeicecreamandmango",
            DICTIONARY1);
SIMPLE_TEST(AllWayToWordBreak, TestSAMPLE2, EXPECTED2, "ilikesamsungmobile",
            DICTIONARY1);
