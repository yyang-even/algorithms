...
auto RemoveBoxes_Memo(const ArrayType &boxes, const int left, int right,
    ...
    memo[left][right][k] =
        RemoveBoxes_Memo(boxes, left, right - 1, 0, memo) +
        (k + 1) * (k + 1);

    for (int i = left; i < right; ++i) {
        if (boxes[i] == boxes[right]) {
            memo[left][right][k] =
                std::max(memo[left][right][k],
                         RemoveBoxes_Memo(boxes, left, i, k + 1, memo) +
                         RemoveBoxes_Memo(boxes, i + 1, right - 1, 0, memo));
        }
    }
...
auto RemoveBoxes_DP(const ArrayType &boxes) {
