...
auto ConstructBinaryTree_InAndPost(const BinaryTree::ArrayType &postorder,
                                   int &post_i, const std::unordered_map<int, int> &inorder_map,
                                   const int left, const int right) {
    if (left > right)
        return BinaryTree::Node::PointerType{};
    ...
    const auto mid = inorder_map.at(root_value);
    root->right =
        ConstructBinaryTree_InAndPost(postorder, post_i, inorder_map, mid + 1, right);
    root->left = ConstructBinaryTree_InAndPost(postorder, post_i, inorder_map, left,
                                               mid - 1);
...
auto ConstructBinaryTree_InAndPost(const BinaryTree::ArrayType &inorder,
    ...
    return ConstructBinaryTree_InAndPost(postorder, post_i, index_map, 0,
                                         inorder.size() - 1);
}
...
SIMPLE_TEST(areIdenticalTrees, TestSAMPLE2, true,
            SAMPLE1, ConstructBinaryTree_InAndPost(SAMPLE1IN, SAMPLE1POST));
