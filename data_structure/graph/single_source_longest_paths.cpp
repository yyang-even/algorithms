 ...
 */
void LengthOfTheLongestPath_Unweighted_DAG_DFS(const
    ...
    for (const auto adjacent_vertex : graph.at(source)) {
        if (not visited_vertices[adjacent_vertex]) {
            LengthOfTheLongestPath_Unweighted_DAG_DFS(graph, adjacent_vertex, visited_vertices,
                                                      longest_paths);
        }

        longest_paths[source] = std::max(longest_paths[source],
                                         1 + longest_paths[adjacent_vertex]);
    }
}
...
auto LengthOfTheLongestPath_Unweighted_DAG_DFS(const std::size_t number_vertices,
    ...
    [&longest_paths](const auto & graph, const auto source, auto & visited_vertices) {
        LengthOfTheLongestPath_Unweighted_DAG_DFS(graph, source, visited_vertices,
                                                  longest_paths);
        return true;
    });
