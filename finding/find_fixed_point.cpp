 ...
 */
auto FindOneFixedPointInArrayWithDuplicates(const ArrayType &elements, const int low,
        const int high) {
    if (high < low) {
        return -1;
    }
    ...
    const auto result_of_left_side = FindOneFixedPointInArrayWithDuplicates(elements, low,
                                     std::min(mid - 1, elements[mid]));
