 ...
 */
auto toString(const ArrayType::value_type expected,
              const ArrayType::value_type current) {
    auto result = std::to_string(expected);
    if (expected < current - 1) {
        result.append("->");
        result.append(std::to_string(current - 1));
    }
...
auto MissingRanges(const ArrayType &elements,
    ...
    for (const auto a_number : elements) {
        ...
        if (expected != a_number) {
            results.push_back(toString(expected, a_number));
        }
        prev = a_number;
    }

    if (prev != upper) {
        results.push_back(toString(prev + 1, upper + 1));
    }
