 ...
 * individual strings.
 */
auto InterleavingsOfTwoStringHelper(const std::string::const_iterator cbegin1,
    ...
    if(cbegin1 == cend1 and cbegin2 == cend2) {
        ...
        if(cbegin1 != cend1){
            *buffer_iter = *cbegin1;
            InterleavingsOfTwoStringHelper(cbegin1 + 1, cend1, cbegin2, cend2, buffer, buffer_iter + 1, outputs);
        }
        if(cbegin2 != cend2){
            *buffer_iter = *cbegin2;
            InterleavingsOfTwoStringHelper(cbegin1, cend1, cbegin2 + 1, cend2, buffer, buffer_iter + 1, outputs);
        }
    }
}
auto InterleavingsOfTwoString(const std::string str1, const std::string str2) {
    ...
    InterleavingsOfTwoStringHelper(str1.cbegin(), str1.cend(), str2.begin(), str2.cend(), buffer, beffuer.begin(), outputs);
...
SIMPLE_BENCHMARK(Permutation, std::string("abcde"), std::string("12345"));

SIMPLE_TEST(Permutation, TestSAMPLE1, EXPECTED1, std::string("ab"), std::string("c"));
SIMPLE_TEST(Permutation, TestSAMPLE2, EXPECTED2, std::string("ab"), std::string("cd"));
