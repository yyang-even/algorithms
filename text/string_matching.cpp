...
auto StringMatcing_OptimizedNaive(const std::string &text, const std::string &pattern) {
    assert(text.size() >= pattern.size());
    assert(std::unordered_set<char>(pattern.cbegin(),
                                    pattern.cend()).size() == pattern.size());
...
auto createTransitionFunctionTable(const std::string &pattern, const int RADIX_D) {
    std::vector<ArrayType> transition_fucntion_table(pattern.size() + 1, ArrayType(RADIX_D,
                                                     0));
...
SIMPLE_TEST(StringMatcing_Naive, TestSAMPLE5, EXPECTED2, "ABABDABACDABABCABAB",
            "ABABCABAB");
...
SIMPLE_TEST(StringMatcing_OptimizedNaive, TestSAMPLE3, EXPECTED3, "ABCEABCDABCEABCD",
            "ABCD");


THE_BENCHMARK(StringMatcing_RabinKarp, "AABAACAADAABAABA", "AABA");

SIMPLE_TEST(StringMatcing_RabinKarp, TestSAMPLE1, EXPECTED1, "AABAACAADAABAABA",
            "AABA");
SIMPLE_TEST(StringMatcing_RabinKarp, TestSAMPLE2, EXPECTED2, "THIS IS A TEST TEXT",
            "TEST");
SIMPLE_TEST(StringMatcing_RabinKarp, TestSAMPLE3, EXPECTED3, "ABCEABCDABCEABCD",
            "ABCD");
SIMPLE_TEST(StringMatcing_RabinKarp, TestSAMPLE4, EXPECTED4, "GEEKS FOR GEEKS",
            "GEEKS");
SIMPLE_TEST(StringMatcing_RabinKarp, TestSAMPLE5, EXPECTED2, "ABABDABACDABABCABAB",
            "ABABCABAB");
SIMPLE_TEST(StringMatcing_RabinKarp, TestSAMPLE6, EXPECTED6, "cabababcababaca",
            "ababaca");
