...
/**
 * @reference   Split a sentence into words in C++
...
auto SplitIntoWords_strtok(std::string sentence) {
    ArrayType outputs;
    for (auto *token = std::strtok(sentence.data(), SPACES); token != nullptr;
...
/**
 * @reference   Count words in a given string
 ...
 * @tags    #tokenizing
 */
auto WordAfterBigram(const std::string &text,
 ...
 * searchWord is a prefix of more than one word, return the index of the first word (minimum index). If
 ...
 * of one or more lowercase English letters and are separated by at least one space. It's guaranteed
 * that text contains at least one word.
 ...
 * increasing from left to right (i.e., other than the last number, each number is strictly smaller than
 ...
 *
 * You are given a string title consisting of one or more words separated by a single space, where each
 * word consists of English letters. Capitalize the string by changing the capitalization of each word
 * such that:
...
/**
 * @reference   Count Mentions Per User
 *              https://leetcode.com/problems/count-mentions-per-user/
 *
 * You are given an integer numberOfUsers representing the total number of users and an array events of
 * size n x 3.
 ...
 *              multiple ids separated by a single whitespace and may contain duplicates. This can
 *              mention even the offline users.
 ...
 * Return an array mentions where mentions[i] represents the number of mentions the user with id i has
 * across all MESSAGE events.
 ...
 * Note that a user can be mentioned multiple times in a single message event, and each mention should
 * be counted separately.
 *
 * @tags    #sorting #hash-table #tokenizing
 */
...
SIMPLE_TEST(SplitIntoWords_strtok,
            TestSAMPLE1,
            EXPECTED1,
            "One two         three\n    four\tfive  ");
...
SIMPLE_TEST(SplitIntoWords_SS, TestSAMPLE1, EXPECTED1, "One two         three\n    four\tfive  ");
...
THE_BENCHMARK(CountWordsInGivenString, "One two         three\n    four\tfive  ");

SIMPLE_TEST(CountWordsInGivenString, TestSAMPLE1, 5, "One two         three\n    four\tfive  ");


THE_BENCHMARK(CountWordsInGivenString_StringStream, "One two         three\n    four\tfive  ");

SIMPLE_TEST(CountWordsInGivenString_StringStream,
            TestSAMPLE1,
            5,
            "One two         three\n    four\tfive  ");
...
THE_BENCHMARK(WordAfterBigram, "alice is a good girl she is a good student", "a", "good");

SIMPLE_TEST(WordAfterBigram,
            TestSAMPLE1,
            EXPECTED1B,
            "alice is a good girl she is a good student",
            "a",
            "good");
SIMPLE_TEST(WordAfterBigram, TestSAMPLE2, EXPECTED2B, "we will we will rock you", "we", "will");
SIMPLE_TEST(WordAfterBigram,
            TestSAMPLE3,
            EXPECTED3B,
...
SIMPLE_TEST(RearrangeSpaces, TestSAMPLE1, "this   is   a   sentence", "  this   is  a sentence ");
SIMPLE_TEST(RearrangeSpaces,
            TestSAMPLE2,
