...
/**
 * @reference   Minimum Distance Between Words of a String
 *              https://www.geeksforgeeks.org/minimum-distance-between-words-of-a-string/
 *
 * Given a string s and two words w1 and w2 that are present in S. The task is to find the minimum
 * distance between w1 and w2. Here distance is the number of steps or words between the first and the
 * second word.
 *
 ...
 * terms of number of words) between them in the file. If the operation will be repeated many times for
 * the same file (but different pairs of words), can you optimize your solution?
 */
unsigned
MinDistanceBetweenWords(std::string text, const std::string_view w1, const std::string_view w2) {
    if (w1 == w2) {
        return 0;
    }

    ArrayType words;
    for (auto *token = std::strtok(text.data(), SPACES); token != nullptr;
...
inline auto MinDistanceBetweenWords_MultiQueries_Hash(const ArrayType &words,
                                                      const std::string_view one_word,
                                                      const std::string_view another) {
    assert(one_word != another);
 ...
 * Given a list of words and two words word1 and word2, return the shortest distance between these two
 * words in the list.
 * word1 and word2 may be the same and they represent two individual words in the list. Note: You may
 * assume word1 and word2 are both in the list.
 */
unsigned MinDistanceBetweenWords_MayBeIdentical(const ArrayType &words,
                                                const std::string_view one_word,
                                                const std::string_view another) {
    if (one_word != another) {
...
unsigned MinDistanceBetweenWords_MayBeIdentical_Short(const ArrayType &words,
                                                      const std::string_view one_word,
                                                      const std::string_view another) {
    int last_one = -1;
    int last_another = -1;
...
SIMPLE_TEST(MinDistanceBetweenWords,
            TestSAMPLE1,
            1,
            "geeks for geeks contribute practice",
            "geeks",
...
SIMPLE_TEST(
    MinDistanceBetweenWords_MultiQueries_Hash, TestSAMPLE1, 3, SAMPLE1, "practice", "coding");
SIMPLE_TEST(
    MinDistanceBetweenWords_MultiQueries_Hash, TestSAMPLE2, 1, SAMPLE1, "makes", "coding");


THE_BENCHMARK(MinDistanceBetweenWords_MayBeIdentical, SAMPLE1, "practice", "coding");

SIMPLE_TEST(
    MinDistanceBetweenWords_MayBeIdentical, TestSAMPLE1, 3, SAMPLE1, "practice", "coding");
SIMPLE_TEST(MinDistanceBetweenWords_MayBeIdentical, TestSAMPLE2, 1, SAMPLE1, "makes", "coding");
SIMPLE_TEST(MinDistanceBetweenWords_MayBeIdentical, TestSAMPLE3, 3, SAMPLE1, "makes", "makes");


THE_BENCHMARK(MinDistanceBetweenWords_MayBeIdentical_Short, SAMPLE1, "practice", "coding");

SIMPLE_TEST(
    MinDistanceBetweenWords_MayBeIdentical_Short, TestSAMPLE1, 3, SAMPLE1, "practice", "coding");
SIMPLE_TEST(
    MinDistanceBetweenWords_MayBeIdentical_Short, TestSAMPLE2, 1, SAMPLE1, "makes", "coding");
SIMPLE_TEST(
    MinDistanceBetweenWords_MayBeIdentical_Short, TestSAMPLE3, 3, SAMPLE1, "makes", "makes");
