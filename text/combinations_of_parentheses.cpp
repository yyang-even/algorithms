...
void CombinationsOfParentheses(const unsigned number_pairs,
    ...
    if (count_close == number_pairs) {
        ...
        if (count_open > count_close) {
            ...
            CombinationsOfParentheses(number_pairs, count_open, count_close + 1, one_combination,
                                      results);
        ...
        if (count_open < number_pairs) {
            one_combination.push_back('{');
            CombinationsOfParentheses(number_pairs, count_open + 1, count_close, one_combination,
                                      results);
            one_combination.pop_back();
        }
    }
