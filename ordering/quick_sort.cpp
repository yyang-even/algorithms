...
auto TestQuickSortSinglyLinkedList(const ArrayType &values) {
    std::forward_list<int> l{values.cbegin(), values.cend()};
    const auto sorted_l = QuickSortSinglyLinkedList(std::move(l));
    return ArrayType{sorted_l.cbegin(), sorted_l.cend()};
}
}//namespace
