 ...
 *
 * Given an integer array of which both first half and second half are sorted. Task is to merge two
 * sorted halves of array into single sorted array.
 *
 * @reference   Sort an Array
 ...
 * @complexity: O(n*lgn)
 */
inline void Merge(const ArrayType::iterator begin,
 ...
 *
 * Merge subarrays in bottom up manner. First merge subarrays of size 1 to create sorted subarrays of
 * size 2, then merge subarrays of size 2 to create sorted subarrays of size 4, and so on.
 */
auto MergeSort_Iterative(ArrayType values) {
    const auto N = values.size();
    for (ArrayType::size_type current_size = 1; current_size < N; current_size *= 2) {
 ...
 *
 * How to modify the algorithm so that merge works in O(1) extra space and algorithm still works in
 * O(n Log n) time.
 */
void Merge_O1(const ArrayType::iterator begin,
              const ArrayType::iterator middle,
              const ArrayType::iterator end,
    ...
    std::transform(L_iter, middle, iter, iter, [max](const auto v1, const auto v2) {
 ...
 * computing device (usually RAM) and instead they must reside in the slower external memory (usually a
 * hard drive). External sorting typically uses a hybrid sort-merge strategy. In the sorting phase,
 * chunks of data small enough to fit in main memory are read, sorted, and written out to a temporary
...
auto MergeSort_SinglyList_Iterative(std::forward_list<int> values) {
    if (not values.empty()) {
        ...
        while (not values.empty()) {
            lists.emplace_back();
            lists.back().splice_after(
                lists.back().cbefore_begin(), values, values.cbefore_begin());
        }
        const auto N = lists.size();
        for (std::size_t gap = 1; gap <= N;) {
            const auto step = gap * 2;
            std::size_t left_index = 0;
            for (auto right_index = left_index + gap; right_index < N and left_index < N;
...
inline void Merge_3Way(const ArrayType::iterator begin,
                       ...
                       const ArrayType::iterator end) {
    MergeKSortedArrays(
        {ArrayType(begin, middle1), ArrayType(middle1, middle2), ArrayType(middle2, end)}, begin);
}
