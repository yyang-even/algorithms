...
static inline auto &operator<<(std::ostream &out, const Rectangle &s) {
    return out << "(left: " << s.left << ", top: " << s.top << ", right: " << s.right
               << ", bottom: " << s.bottom << ")";
}


static inline constexpr auto Middle(const Rectangle &a_square) {
    return Point {(a_square.left + a_square.right) / 2.0, (a_square.top + a_square.bottom) / 2.0};
}
 ...
 *
 * Given coordinates of bottom-left and top-right corners of a rectangle. Check if a point (x, y) lies
 * inside this rectangle or not.
 *
 * @tags    #geometry #rectangle
 */
static inline constexpr auto Contains(const Rectangle &a_square, const Point &a_point) {
...
static inline constexpr auto
Area(const double left, const double top, const double right, const double bottom) {
